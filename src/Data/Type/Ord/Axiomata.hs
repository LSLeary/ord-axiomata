...
class TotalOrder o => BoundedBelow o where
  ...
  lbLeqAll :: Sing o a -> (LowerBound o <=? a) :~: True

instance BoundedBelow Nat where
  type LowerBound Nat = 0
  lowerBound = natSing
  lbLeqAll n@SNat = case cmpNat lowerBound n of
    LTI -> Refl
    EQI -> Refl
    GTI -> error "lbLeqAll @Nat: n < 0"

instance BoundedBelow Char where
  type LowerBound Char = '\NUL'
  lowerBound = charSing
  lbLeqAll n@SChar = case cmpChar lowerBound n of
    LTI -> Refl
    EQI -> Refl
    GTI -> error "lbLeqAll @Char: c < '\\NUL'"

instance BoundedBelow Symbol where
  type LowerBound Symbol = ""
  lowerBound = symbolSing
  lbLeqAll n@SSymbol = case cmpSymbol lowerBound n of
    LTI -> Refl
    EQI -> Refl
    GTI -> error "lbLeqAll @Symbol: s < \"\""
...
class TotalOrder o => BoundedAbove o where
  ...
  allLeqUB :: Sing o a -> (a <=? UpperBound o) :~: True
...
minMono a b c d = case c <?=?> d of
  LTI -> case minLeqLeft a b of
    Refl -> transTO (minTO a b) a c
  EQI -> case minLeqLeft a b of
    Refl -> transTO (minTO a b) a c
  GTI -> case minLeqRight a b of
...
maxMono a b c d = case a <?=?> b of
  LTI -> case rightLeqMax c d of
    Refl -> transTO b d (maxTO c d)
  EQI -> case rightLeqMax c d of
    Refl -> transTO b d (maxTO c d)
  GTI -> case leftLeqMax c d of
